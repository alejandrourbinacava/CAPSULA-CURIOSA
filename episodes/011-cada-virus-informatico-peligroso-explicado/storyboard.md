...
> 02s solo ejecuta lo escrito. Cada idea con su icono. `CHAP "Nombre" icono` fija el HUD. Fotos reales en los hitos.
...
```
+0.0  ICO  worm-code      @center  pop
+3.4  TXT  "unas líneas de código" @cap  handwrite  md
```
...
+2.4  TXT  "los más peligrosos" @cap  stamp  lg  red
```
...
+3.0  TXT  "de broma a arma de guerra" @cap  handwrite  md
```
...
```
+0.0  ICO  types-malware  @left   fade
+3.6  TXT  "virus · gusano · troyano" @right  handwrite  md
```
...
```
+0.0  ICO  worm-code      @center  pop
+2.0  TXT  "casi todos: GUSANOS" @cap  stamp  md  red
```
...
```
+0.0  IMG  photo-retro-pc @center  pop
...
```
+0.0  ICO  old-computer   @left   fade
+2.0  TXT  "el 10% de internet" @right  stamp  lg  red
```
...
```
+0.0  ICO  worm-code      @left   fade
+1.6  TXT  "recuérdalo" @right  handwrite  md
```
...
```
+0.0  ICO  floppy-word    @center  fade
...
```
+0.0  ICO  floppy-word    @left   fade
+3.6  TXT  "se reenvía a 50 contactos" @right  handwrite  md
```
...
+2.8  TXT  "tumbó los servidores" @cap  stamp  md  red
```

### BEAT 18 · 02:09 · T1
Narración: *"Incluida la mismísima Microsoft, que tuvo que apagar su sistema."*
```
+0.0  ICO  server-down    @left   fade
+1.6  TXT  "hasta Microsoft cayó" @right  stamp  lg  red
```
...
```
+0.0  ICO  love-email     @center  fade
+3.2  TXT  "¿quién no lo abre?" @cap  handwrite  md
```
...
```
+0.0  ICO  click-warning  @center  pop
+2.8  TXT  "un clic = desastre" @cap  stamp  md  red
```
...
```
+0.0  ICO  money-burning  @center  pop
...
```
+0.0  ICO  love-email     @left   fade
+4.0  TXT  "Pentágono y CIA cayeron" @right  handwrite  md
```
...
+2.6  TXT  "no había ley que lo castigara" @right  handwrite  md
```
...
+0.0  ICO  defaced-web    @center  pop
...
```
+0.0  ICO  defaced-web    @center  fade
...
```
+0.0  ICO  white-house    @center  pop
...
```
+0.0  ICO  worm-code      @left   fade
+2.8  TXT  "22 minutos al nº1" @right  stamp  lg  red
```
...
```
+0.0  ICO  tiny-packet    @center  fade
...
```
+0.0  ICO  tiny-packet    @left   fade
+3.4  TXT  "x2 cada 8,5 segundos" @right  stamp  lg  red
```

### BEAT 35 · 04:16 · T1
Narración: *"Léelo otra vez. Cada ocho segundos, el doble."*
```
+0.0  ICO  tiny-packet    @center  pop
...
```
+0.0  ICO  atm-down       @left   pop
+2.6  ICO  nuclear-plant  @right  pop
...
+0.0  ICO  email-storm    @center  pop
...
```
+0.0  ICO  email-storm    @center  fade
...
```
+0.0  ICO  email-storm    @left   fade
+3.6  TXT  "1 de cada 4 correos" @right  stamp  lg  red
```
...
```
+0.0  ICO  warship        @center  pop
+1.8  TXT  "hasta la marina francesa" @cap  handwrite  md
```
...
+2.8  TXT  "¿para qué? nadie lo supo" @right  stamp  md  red
```
...
```
+0.0  ICO  centrifuge     @left   fade
+3.0  TXT  "un ARMA de gobiernos" @right  stamp  lg  red
```
...
```
+0.0  IMG  photo-stuxnet  @center  pop
...
```
+0.0  ICO  gear-breaking  @center  pop
...
```
+0.0  ICO  fake-screen    @center  fade
...
```
+0.0  ICO  worm-code      @left   fade
+3.2  TXT  "código que rompe metal" @right  handwrite  md
```
...
```
+0.0  ICO  nuclear-plant  @center  pop
...
```
+0.0  ICO  centrifuge     @center  fade
...
```
+0.0  ICO  ransom-lock    @center  fade
...
```
+0.0  IMG  photo-wannacry @center  pop
...
```
+0.0  ICO  stolen-key     @left   fade
+4.0  TXT  "la llave se escapó" @right  handwrite  md
```
...
```
+0.0  ICO  ransom-lock    @center  fade
+2.8  TXT  "fingía pedir rescate" @cap  handwrite  md
```
...
```
+0.0  ICO  old-computer   @a   pop
+2.2  ICO  money-burning  @b   pop
+4.0  ICO  centrifuge     @c   pop
```

### BEAT 74 · 09:31 · T1
Narración: *"Cada uno de estos virus obligó al mundo a protegerse un poco más."*
```
+0.0  ICO  shield-lock    @center  pop
+2.0  TXT  "nos hicieron protegernos" @cap  handwrite  md
```
...
```
+0.0  ICO  shield-lock    @left   fade
+3.6  TXT  "antivirus y cortafuegos" @right  handwrite  md
```
...
```
+0.0  ICO  shield-lock    @center  fade
...
```
+0.0  ICO  love-email     @center  pop
+4.0  TXT  "cuidado con ese correo" @cap  stamp  md  red
```
