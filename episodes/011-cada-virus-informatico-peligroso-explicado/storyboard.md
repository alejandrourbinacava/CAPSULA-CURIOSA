...
> LIENZO ACUMULATIVO: los iconos de cada sección se apilan (rejilla auto) y se limpian al cambiar de CHAP.
> Un icono ÚNICO por cada cosa mencionada, sincronizado con ~palabra. Fotos reales en los hitos.
...
```
+0.0  ICO  code-lines     @center  ~líneas   pop
+2.0  ICO  hospital       @center  ~hospitales  pop
+3.4  ICO  factory        @center  ~empresas  pop
+5.0  ICO  nuclear-plant  @center  ~nuclear   pop
...
```
+0.0  ICO  virus-skull    @center  ~virus  pop
...
```
+0.0  ICO  hacker         @center  ~funcionaban  pop
+1.6  ICO  dollar-sign    @center  ~dinero   pop
+3.2  ICO  student        @center  ~estudiantes  pop
+4.4  ICO  weapon-war     @center  ~armas    stamp
...
```
+0.0  ICO  types-malware  @center  ~aclaración  pop
...
```
+0.0  ICO  bug-error      @center  ~archivo  pop
+1.8  ICO  worm-code      @center  ~gusano   pop
+3.6  ICO  thief          @center  ~troyano  pop
...
```
+0.0  ICO  spread-world   @center  ~extendieron  pop
...
+0.0  ICO  calendar       @center  ~ochenta  pop
+2.6  TXT  "Gusano Morris · 1988" @cap  stamp  lg
```
...
```
+0.0  ICO  university     @center  ~universidad  pop
+2.2  IMG  photo-retro-pc @right   ~ordenadores  pop
...
```
+0.0  ICO  spread-world   @center  ~internet  pop
...
```
+0.0  ICO  bug-error      @center  ~error    pop
+2.2  ICO  copy-copy      @center  ~copiara  pop
+3.6  ICO  server-down    @center  ~saturando  pop
+4.6  TXT  "un error, no maldad" @cap  handwrite  md
```
...
```
+0.0  ICO  law-book       @center  ~condenada  pop
...
```
+0.0  ICO  worm-code      @center  ~gusano  pop
+2.8  ICO  code-lines     @center  ~programa  pop
...
```
+0.0  TXT  "recuérdalo" @cap  handwrite  md
```
...
+0.0  ICO  calendar       @center  ~años  pop
...
```
+0.0  ICO  floppy-word    @center  ~documento  pop
+2.4  ICO  address-book   @center  ~libreta    pop
+3.6  ICO  envelope-open  @center  ~contactos  pop
+4.4  TXT  "se reenvía a 50 contactos" @cap  handwrite  md
```

### BEAT 17 · 02:02 · T1
Narración: *"Cincuenta correos por víctima. La cosa crecía tan rápido que tumbó los servidores de correo de empresas enteras."*
```
+0.0  ICO  inbox-flood    @center  ~correos  pop
+1.8  ICO  server-down    @center  ~servidores  pop
...
```
+0.0  ICO  database       @center  ~sistema  pop
...
+0.0  ICO  envelope-open  @center  ~llega  pop
...
```
+0.0  ICO  love-email     @center  ~correo    pop
+2.4  ICO  curiosity-face @center  ~curiosidad  pop
...
+2.0  ICO  destruction-bomb @center ~destruía  pop
...
```
+0.0  IMG  photo-iloveyou @right  ~ordenadores  pop
+0.0  ICO  spread-world   @center  ~infectó  pop
...
```
+0.0  ICO  pentagon       @center  ~pentágono  pop
+2.4  ICO  hacker         @center  ~cia  pop
...
```
+0.0  ICO  law-book       @center  ~impunes  pop
...
```
+0.0  ICO  virus-skull    @center  ~virus  pop
+2.4  TXT  "no había ley que lo castigara" @cap  handwrite  md
```
...
+0.0  ICO  calendar       @center  ~año  pop
+1.6  ICO  defaced-web    @center  ~code  pop
...
```
+0.0  ICO  server-down    @center  ~servidores  pop
...
+1.6  ICO  crosshair-target @center ~ataque  pop
+2.4  TXT  "atacó la Casa Blanca" @cap  stamp  lg  red
```
...
```
+0.0  ICO  worm-code      @center  ~nimda  pop
+2.0  ICO  five-arrows    @center  ~cinco  pop
...
```
+0.0  ICO  stopwatch-fast @center  ~velocidad  pop
+2.4  ICO  spread-world   @center  ~extendido  pop
...
```
+0.0  ICO  data-packet    @center  ~paquete  pop
...
```
+0.0  ICO  lightning-speed @center  ~velocidad  pop
+2.4  ICO  copy-copy       @center  ~doblaba  pop
...
```
+0.0  ICO  spread-world   @center  ~doble  pop
...
```
+0.0  ICO  database       @center  ~vulnerable  pop
...
```
+0.0  ICO  atm-down       @center  ~cajeros  pop
+2.6  ICO  nuclear-plant  @center  ~nuclear  pop
...
+0.0  ICO  dollar-sign    @center  ~dinero  pop
...
```
+0.0  ICO  email-storm    @center  ~correo  pop
+1.8  ICO  lightning-speed @center  ~rápido  pop
...
```
+0.0  ICO  pie-quarter    @center  ~cuatro  pop
+2.4  ICO  spread-world   @center  ~mundo   pop
...
+0.0  ICO  fear-face      @center  ~inquietante  pop
...
```
+0.0  ICO  zombie-pc      @center  ~ordenadores  pop
+2.8  ICO  question-army  @center  ~ejército  pop
...
```
+0.0  ICO  warship        @center  ~marina  pop
+1.4  ICO  france-flag    @center  ~francesa  pop
+2.2  ICO  hospital       @center  ~hospitales  pop
+2.8  TXT  "marina y hospitales" @cap  handwrite  md
```

### BEAT 45 · 05:29 · T1
Narración: *"Pero lo que pone los pelos de punta es otra cosa. Que nunca se supo del todo para qué servía."*
```
+0.0  ICO  question-army  @center  ~supo  pop
...
```
+0.0  ICO  fear-face      @center  ~mundo  pop
+3.0  TXT  "reclutado y abandonado" @cap  handwrite  md
```
...
```
+0.0  ICO  question-army  @center  ~debaten  pop
...
+0.0  ICO  calendar       @center  ~diez  pop
...
```
+0.0  ICO  student        @center  ~estudiantes  pop
+1.4  ICO  thief          @center  ~ladrones  pop
+3.0  ICO  weapon-war     @center  ~arma  stamp
...
```
+0.0  ICO  iran-map       @center  ~irán  pop
+2.2  IMG  photo-stuxnet  @right   ~centrifugadoras  pop
+3.4  ICO  uranium        @center  ~uranio  pop
+3.8  TXT  "centrifugadoras de Irán" @cap  handwrite  md
```
...
```
+0.0  ICO  operator       @center  ~operarios  pop
+2.0  ICO  fake-screen    @center  ~pantallas  pop
...
```
+0.0  ICO  destruction-bomb @center  ~destrucción  pop
...
```
+0.0  ICO  code-lines     @center  ~código  pop
+2.4  TXT  "código que rompe metal" @cap  handwrite  md
```
...
```
+0.0  ICO  power-grid     @center  ~electricidad  pop
+2.4  ICO  water-tap      @center  ~agua  pop
+3.2  ICO  factory        @center  ~fábricas  pop
...
+0.0  ICO  encrypted-files @center  ~secuestro  pop
...
```
+0.0  ICO  ransom-note    @center  ~rescate  pop
+2.4  ICO  encrypted-files @center  ~cifraba  pop
+4.2  ICO  dollar-sign    @center  ~dinero  pop
+4.6  TXT  "ransomware = secuestro" @cap  stamp  md  red
```

### BEAT 60 · 07:33 · T1
Narración: *"En un solo fin de semana, cifró doscientos mil ordenadores en ciento cincuenta países."*
```
+0.0  IMG  photo-wannacry @right  ~ordenadores  pop
+0.0  ICO  spread-world   @center  ~países  pop
...
```
+0.0  ICO  hospital       @center  ~salud  pop
+1.8  ICO  uk-flag        @center  ~británico  pop
+2.8  ICO  ambulance      @center  ~ambulancias  pop
+3.2  TXT  "paralizó hospitales" @cap  stamp  lg  red
```
...
+3.0  ICO  nsa-building   @center  ~agencia  pop
...
```
+0.0  ICO  padlock-broken @center  ~llave  pop
...
```
+0.0  ICO  kill-switch    @center  ~interruptor  pop
...
+0.0  ICO  money-burning  @center  ~caro  pop
...
```
+0.0  ICO  ransom-note    @center  ~rescate  pop
+2.6  ICO  thief          @center  ~trampa  pop
...
```
+0.0  ICO  ukraine-flag   @center  ~ucrania  pop
+2.4  ICO  spread-world   @center  ~mundo  pop
...
```
+0.0  IMG  photo-cargo    @right   ~maersk  pop
+0.0  ICO  delivery-truck @center  ~mensajería  pop
+2.0  ICO  pill-pharma    @center  ~farmacéutica  pop
...
```
+0.0  ICO  crosshair-target @center  ~objetivo  pop
+3.6  TXT  "daño colateral masivo" @cap  handwrite  md
```
...
```
+0.0  CHAP ""
+0.0  ICO  hacker         @center  ~evolución  pop
...
```
+0.0  ICO  student        @center  ~estudiante  pop
+2.0  ICO  thief          @center  ~ladrones  pop
+2.8  ICO  fame-star      @center  ~fama  pop
+4.4  ICO  weapon-war     @center  ~armas  stamp
```
...
```
+0.0  ICO  shield-lock    @center  ~antivirus  pop
+2.4  ICO  virus-skull    @center  ~ataques  pop
...
```
+0.0  ICO  brain-curiosity @center  ~curiosidad  pop
+2.4  ICO  fear-face       @center  ~miedo  pop
...
```
+0.0  ICO  shield-lock    @center  ~tecnología  pop
+2.0  ICO  click-warning  @center  ~débil  pop
...
```
+0.0  ICO  love-email     @center  ~correo  pop
+2.6  ICO  bug-error      @center  ~archivo  pop
